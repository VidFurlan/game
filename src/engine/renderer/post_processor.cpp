...
PostProcessor::PostProcessor(Shader shader, unsigned int width, unsigned int height)
    : mPostProcessingShader(shader), mTexture(), mWidth(width), mHeight(height) {
	...
	this->mTexture.Generate(width, height, NULL);
	...
	this->InitRenderData();
	this->mPostProcessingShader.SetInteger("scene", 0, true);
	...
	glUniform2fv(glGetUniformLocation(this->mPostProcessingShader.ID, "offsets"), 9, (float*)offsets);
	int edge_kernel[9] = {
	    -1, -1, -1, -1, 8, -1, -1, -1, -1};
	glUniform1iv(glGetUniformLocation(this->mPostProcessingShader.ID, "edge_kernel"), 9, edge_kernel);
	float blur_kernel[9] = {
	    1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f, 2.0f / 16.0f, 4.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f};
	glUniform1fv(glGetUniformLocation(this->mPostProcessingShader.ID, "blur_kernel"), 9, blur_kernel);
}
...
void PostProcessor::Render(float time) {
	this->mPostProcessingShader.Use();
	...
	this->mPostProcessingShader.SetFloat("mask_amount", 0.0f);
